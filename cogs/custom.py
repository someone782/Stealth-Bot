...
class custom(commands.Cog):
    ...
    async def bonk(self, ctx, member : discord.Member=None):
        ...
        await ctx.reply(f"{member.mention} {message} {ctx.author.mention}. What a loser!")

    # <@547059830062448670>

    @commands.command(help="Tells you to have some pickles!")
    async def hungry(self, ctx):
        await ctx.reply(f"then have some pickles!")

    # <@748105241857097759>

    @commands.command(help="Tells you to shut up cause stars is sleeping")
    async def stars(self, ctx):
        await ctx.reply(f"shut up, stars is sleeping")
    ...
    async def luck(self, ctx, member : discord.Member=None):
        ...
        responses = ["has bad luck!",
                    "has good luck!"]
        await ctx.reply(f"{member.mention} {random.choice(responses)}")
        return
    ...
    async def fact(self, ctx, member : discord.Member=None):
        if(member == None):
            member = ctx.author
        await ctx.reply(f"Fun fact about {member.mention}: they're a loser.")
    ...
    #     else:
    #         await ctx.reply(f"https://tenor.com/view/burger-eating-burbger-asmr-sussy-gif-21505937")
    ...
    async def ffmpeg(self, ctx):
        ...
        await ctx.reply(f"is very slow")
